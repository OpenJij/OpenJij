...
namespace openjij {
  namespace graph {
     ...
     * @param relabel re-label variable_labels. Disable the option if the model has specified topology (such as square
     * lattice or chimera model). if the option is disabled, IndexType of JSON must be an integer.
     *
...
#pragma omp parallel for
      ...
      return std::tuple<cimod::PolynomialKeyList<Index>, cimod::PolynomialValueList<FloatType>>(
          poly_key_list, poly_value_list );
    }
