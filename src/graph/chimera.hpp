  ...
  namespace graph {
    ...
    private:
      ...
      inline void _checkpair( Index idx1, Index idx2 ) const {
        ...
        if ( !( ( r1 == r2 && std::abs( c1 - c2 ) == 1 && i1 == i2 && i1 >= 4 ) || // horizontal connect
          ...
          throw std::runtime_error(
              "invalid index pair " + std::to_string( idx1 ) + " " + std::to_string( idx2 ) + " inserted in Chimera" );
        }
      }
    ...
    public:
      ...
      Chimera( std::size_t num_row, std::size_t num_column, FloatType init_val = 0 ) :
          Sparse<FloatType>( num_row * num_column * _num_in_chimera, 6 + 1 ), _init_val( init_val ), _num_row( num_row ),
