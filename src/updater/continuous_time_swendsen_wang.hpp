...
namespace openjij {
  namespace updater {
    ...
    struct ContinuousTimeSwendsenWang<system::ContinuousTimeIsing<graph::Sparse<FloatType>>> {
      ...
      static void update(
          system::ContinuousTimeIsing<GraphType>& system,
        ...
        for ( graph::Index i = 0; i < num_spin; i++ ) {
          ...
          auto cuts
              = generate_poisson_points( 0.5 * system.gamma * ( 1.0 - parameter.s ), parameter.beta, random_number_engine );
        ...
        for ( graph::Index i = 0; i < num_spin; i++ ) {
          for ( typename CTIsing::SparseMatrixXx::InnerIterator it( system.interaction, i ); it; ++it ) {
            ...
            const auto bonds
                = generate_poisson_points( std::abs( 0.5 * J * parameter.s ), parameter.beta, random_number_engine );
        ...
        for ( const auto& cluster : cluster_map ) {
          ...
          if ( urd( random_number_engine ) < probability ) {
            ...
            for ( auto node : cluster.second ) {
              auto i
                  = std::distance( index_helper.begin(), std::upper_bound( index_helper.begin(), index_helper.end(), node ) )
                    - 1;
      ...
      create_timeline( const std::vector<CutPoint>& old_timeline, const std::vector<TimeType>& cuts ) {
        ...
        while ( true ) {
          ...
          if ( cuts_itr == cuts.end() ) {
            std::for_each(
                timeline_itr, concatenated_timeline.end(), [ & ]( CutPoint it ) { new_timeline.push_back( it ); } );
            break;
          }
          ...
          if ( timeline_itr == concatenated_timeline.end() ) {
            std::for_each(
                cuts_itr, cuts.end(), [ & ]( TimeType it ) { new_timeline.push_back( CutPoint( it, current_spin ) ); } );
            break;
          }
       ...
       */
      static std::vector<CutPoint>
      create_timeline_easy( const std::vector<CutPoint>& old_timeline, const std::vector<TimeType>& cuts ) {
        /* remove redundant cuts*/
        std::vector<CutPoint> new_timeline;
        auto current_spin = old_timeline.back().second;
        for ( auto cut_point : old_timeline ) {
        ...
        for ( auto cut : cuts ) {
          ...
          auto itr = std::upper_bound( new_timeline.begin(), new_timeline.end(), dummy_cut, first_lt );
      ...
      generate_poisson_points( const TimeType lambda, const TimeType beta, RandomNumberEngine& random_number_engine ) {
