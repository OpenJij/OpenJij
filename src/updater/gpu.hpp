...
namespace openjij {
  namespace updater {
     ...
     *
     */
    template<typename FloatType, std::size_t rows_per_block, std::size_t cols_per_block, std::size_t trotters_per_block>
    struct GPU<system::ChimeraTransverseGPU<FloatType, rows_per_block, cols_per_block, trotters_per_block>> {
      ...
      inline static void update(
        ...
        random_number_engine.generate_uniform(
            system.info.rows * system.info.cols * system.info.trotters * system.info.chimera_unitsize, system.dev_random );
        // do metropolis
        system::chimera_cuda::metropolis_interface<FloatType, rows_per_block, cols_per_block, trotters_per_block>(
            0,
            system.spin.get(),
            system.dev_random.get(),
     ...
     *
     */
    template<typename FloatType, std::size_t rows_per_block, std::size_t cols_per_block>
    struct GPU<system::ChimeraClassicalGPU<FloatType, rows_per_block, cols_per_block>> {
      ...
      inline static void update(
        ...
        return GPU<typename CIsing::Base>::update(
            system, random_number_engine, utility::TransverseFieldUpdaterParameter( parameter.beta, 1 ) );
      }
    };
