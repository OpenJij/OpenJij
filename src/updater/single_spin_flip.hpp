...
namespace openjij {
  namespace updater {
      ...
      inline static void update(
        ...
        for ( std::size_t index = 0; index < system.num_spins; ++index ) {
          ...
          if ( system.dE( index ) <= 0 || std::exp( -parameter.beta * system.dE( index ) ) > urd( random_number_engine ) ) {
            // update dE
            system.dE
                += 4 * system.spin( index ) * ( system.interaction.row( index ).transpose().cwiseProduct( system.spin ) );
    ...
    struct SingleSpinFlip<system::TransverseIsing<GraphType>> {
       ...
       * @param parameter parameter object including inverse temperature \f\beta:=(k_B T)^{-1}\f and transverse magnetic
       * field \f\s\f
       *
       * @return energy difference \f\Delta E\f
       */
...
#pragma omp parallel for
    ...
    private:
      inline static void
      do_calc( QIsing& system, const utility::TransverseFieldUpdaterParameter& parameter, size_t i, size_t t, FloatType B ) {
        ...
        if ( dE < 0 || exp( -dE ) > system.rand_pool( i, t ) ) {
          ...
          system.dE.col( t )
              += 4 * spins( i, t ) * ( system.interaction.row( i ).transpose().cwiseProduct( spins.col( t ) ) );
          system.dE( i, t ) *= -1;
    ...
    //! @brief Single spin flip for Ising models with polynomial interactions and polynomial unconstrained binary
    //! optimization models.
    ...
    struct SingleSpinFlip<system::ClassicalIsingPolynomial<GraphType>> {
      ...
      //! @brief Operate single spin flip for Ising models with polynomial interactions and polynomial unconstrained binary
      //! optimization models.
      ...
      //! @param parameter const utility::ClassicalUpdaterParameter&. Parameter object including inverse temperature
      //! \f\beta:=(k_B T)^{-1}\f.
      template<typename RandomNumberEngine>
      inline static void update(
          ClPIsing& system,
      ...
      //! \f\beta:=(k_B T)^{-1}\f.
      template<typename RandomNumberEngine>
      inline static void update_spin(
          ClPIsing& system,
          RandomNumberEngine& random_number_engine,
          const utility::ClassicalUpdaterParameter& parameter ) {
        ...
        for ( const auto& index_spin : system.get_active_variables() ) {
          if ( system.dE( index_spin ) <= 0.0
               || std::exp( -parameter.beta * system.dE( index_spin ) ) > urd( random_number_engine ) ) {
      ...
      //! \f\beta:=(k_B T)^{-1}\f.
      template<typename RandomNumberEngine>
      inline static void update_binary(
          ClPIsing& system,
          RandomNumberEngine& random_number_engine,
          const utility::ClassicalUpdaterParameter& parameter ) {
        ...
        for ( const auto& index_binary : system.get_active_variables() ) {
          if ( system.dE( index_binary ) <= 0.0
               || std::exp( -parameter.beta * system.dE( index_binary ) ) > urd( random_number_engine ) ) {
