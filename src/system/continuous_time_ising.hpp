  ...
  namespace system {
    ...
    struct ContinuousTimeIsing<graph::Sparse<FloatType>> {
      ...
      ContinuousTimeIsing(
          ...
          interaction( utility::gen_matrix_from_graph<Eigen::RowMajor>( init_interaction ) ), gamma( gamma ) {
      ...
      ContinuousTimeIsing( const graph::Spins& init_spins, const GraphType& init_interaction, const double gamma ) :
          ContinuousTimeIsing( convert_to_spin_config( init_spins ), init_interaction, gamma ) {
    ...
    private:
      ...
      static SpinConfiguration convert_to_spin_config( const graph::Spins& spins ) {
        ...
        for ( auto spin : spins ) {
          spin_config.push_back(
              std::vector<CutPoint>{ CutPoint( TimeType(), spin ) } ); // TimeType() is zero value of the type
        }
    ...
    public:
      ...
      size_t get_temporal_spin_index( graph::Index site_index, TimeType time_point ) const {
        ...
        auto found_itr = std::upper_bound( timeline.begin(), timeline.end(), dummy_cut, first_lt );
    ...
    make_continuous_time_ising( const graph::Spins& init_spins, const GraphType& init_interaction, double gamma ) {
