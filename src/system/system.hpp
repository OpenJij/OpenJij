...
namespace openjij {
  namespace system {
    ...
    /**
     * @brief meta function to check whether system type is derived from classical_system or quantum_system.
     * If system type is derived from neither classical_system nor quantum_system, returned type will be std::nullptr_t.
     *
     * @tparam System system
     */
    template<typename System>
    struct get_classical_or_quantum_system_type {
      using type = typename utility::get_base_class<typename System::system_type, classical_system, quantum_system>::type;
    };

    /**
     * @brief meta function to check whether system type is derived from mc_system, single_command_system, or
     * real_time_dynamics_system. If system type is derive from none of these systems, returned type will be std::nullptr_t.
     *
     * @tparam System system
     */
    template<typename System>
    struct get_base_system_type {
      using type = typename utility::
          get_base_class<typename System::system_type, mc_system, single_command_system, realtime_dynamics_system>::type;
    };
