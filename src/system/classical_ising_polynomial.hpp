...
namespace openjij {
  namespace system {
    ...
    //! @brief ClassicalIsingPolynomial class, which is a system to solve higher order unconstrained binary optimization
    //! (HUBO) problems with vartype being "SPIN" or "BINARY"
    ...
    public:
      ...
      ClassicalIsingPolynomial(
          const graph::Spins &init_variables,
          const graph::Polynomial<FloatType> &poly_graph,
          const cimod::Vartype init_vartype ) :
          num_variables( poly_graph.size() ),
          variables( init_variables ), vartype( init_vartype ) {
      ...
      ClassicalIsingPolynomial(
          ...
          num_variables( poly_graph.size() ),
          variables( init_variables ), vartype( ConvertVartype( init_vartype ) ) {
        cimod::CheckVariables( variables, vartype );
        SetInteractions( poly_graph );
        SetAdj();
        ResetZeroCount();
      ...
      ClassicalIsingPolynomial( const graph::Spins &init_variables, const nlohmann::json &j ) :
          ...
          vartype( j.at( "vartype" ) == "SPIN" ? cimod::Vartype::SPIN : cimod::Vartype::BINARY ) {
        cimod::CheckVariables( variables, vartype );
...
#pragma omp parallel for
      ...
      //! @brief Get the PolynomialValueList object, which is the list of the values of the polynomial interactions as
      ...
      //! @brief Get the PolynomialKeyList object, which is the list of the indices of the polynomial interactions as
      ...
      //! @brief Get the adjacency list, which is the list of the indices of polynomial interactions including specific
      //! spin/binary.
    ...
    private:
      ...
      //! @brief The number of variables taking the zero in each interaction. Note that this variable is used when the
      //! model's type is BINARY.
      std::vector<int64_t> zero_count_;

      //! @brief The sign of product of spin variables in each interaction. Note that this variable is used when the model's
      //! type is SPIN.
...
#pragma omp parallel for
    ...
    //! @tparam GraphType
    //! @param init_variables const graph::Spins& or const graph::Binaries& (both are equal). The initial spin/binarie
    //! configulations.
    //! @param poly_graph graph::Polynomial<FloatType>& (Polynomial graph class). The initial interacrtions.
    //! @param init_vartype const cimod::Vartype. The model's variable type. SPIN or BINARY.
    template<typename GraphType>
    auto make_classical_ising_polynomial(
        const graph::Spins &init_variables,
