...
namespace openjij {
  namespace system {
    ...
    //! @brief KLocalPolynomial class, which is a system to solve higher order unconstrained binary optimization (HUBO)
    //! problems with vartype being "BINARY"
    ...
    public:
      ...
      KLocalPolynomial( const graph::Binaries &init_binaries, const graph::Polynomial<FloatType> &poly_graph ) :
          num_binaries( init_binaries.size() ), binaries( init_binaries ), binaries_v_( init_binaries ) {
...
#pragma omp parallel for
      ...
      //! @details Note that this function changes the internal state of KLocalPolynomial system. This function virtually
      //! update the system by k-local update.
    ...
    private:
      ...
      ///------------------------------------------------------------------------------------------------------------------------------------------------------
      ///----------------The following member variables are used to virtually update the system for k-local
      ///update----------------
      ///------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      //! @brief The list of virtually updated the number of variables taking the zero in each interaction, which is used to
...
#pragma omp parallel for
      ...
      void SetAdj() {
        ...
        auto compare_value
            = [ this ]( const int64_t i1, const int64_t i2 ) { return poly_value_list_[ i1 ] < poly_value_list_[ i2 ]; };
...
#pragma omp parallel for
      ...
      std::pair<std::vector<graph::Index>, FloatType> FindMinInteraction( const FloatType threshold = 0.0 ) const {
        ...
        for ( std::size_t i = 0; i < poly_key_list_.size(); ++i ) {
          if ( poly_value_list_[ i ] != 0.0 && std::abs( poly_value_list_[ i ] ) < std::abs( min_val )
