...
namespace openjij {
  namespace system {
     ...
     * \f[
     * H(s) = s\left(\sum_{i<j}J_{ij} \sigma_i^z \sigma_j^z + \sum_{i}h_{i} \sigma_i^z \right) + \Gamma (1-s)\sum_i
     * \sigma_i^x, \f] where \f$ \Gamma \f$ shows the strength of transverse field (referred to as `gamma`) and \f$ s \f$
     * shows the annealing schedule (referred to as `s`).
     *
    ...
    struct TransverseIsing<graph::Dense<FloatType>> {
      ...
      TransverseIsing(
          ...
          interaction( init_interaction.get_interactions() ), num_classical_spins( init_trotter_spins[ 0 ].size() ),
          gamma( gamma ) {
       ...
       */
      TransverseIsing(
          const graph::Spins& init_classical_spins,
          const graph::Dense<FloatType>& init_interaction,
          const FloatType gamma,
      ...
      inline void reset_dE() {
        ...
        // diffE = s * (beta/num_trotter_slices) * this->dE + (1/2.) * log(tanh(beta* gamma * (1.0-s) /num_trotter_slices)) *
        // this->dEtrot
      }
