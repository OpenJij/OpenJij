...
namespace openjij {
  namespace system {
    ...
    namespace chimera_cuda {
      ...
      glIdx_x( ChimeraInfo info, std::uint64_t r, std::uint64_t c, std::uint64_t i, std::uint64_t t ) {
        assert( r < info.rows );
        assert( c < info.cols );
       ...
       */
      __host__ __device__ __forceinline__ std::uint64_t
      glIdx_y( ChimeraInfo info, std::uint64_t r, std::uint64_t c, std::uint64_t i, std::uint64_t t ) {
        assert( r < info.rows );
        assert( c < info.cols );
        assert( i < info.chimera_unitsize );
        assert( t < info.trotters );
       ...
       */
      __host__ __device__ __forceinline__ std::uint64_t
      glIdx_z( ChimeraInfo info, std::uint64_t r, std::uint64_t c, std::uint64_t i, std::uint64_t t ) {
        assert( r < info.rows );
        assert( c < info.cols );
        assert( i < info.chimera_unitsize );
        assert( t < info.trotters );
       ...
       */
      __host__ __device__ __forceinline__ std::uint64_t
      glIdx( ChimeraInfo info, std::uint64_t r, std::uint64_t c, std::uint64_t i, std::uint64_t t ) {
        return ( info.chimera_unitsize * info.cols * info.rows ) * glIdx_z( info, r, c, i, t )
               + ( info.chimera_unitsize * info.cols ) * glIdx_y( info, r, c, i, t ) + glIdx_x( info, r, c, i, t );
      }
      ...
      __host__ __device__ __forceinline__ std::uint64_t
      bkIdx( ChimeraInfo info, std::uint64_t b_r, std::uint64_t b_c, std::uint64_t i, std::uint64_t b_t ) {
        return ( info.chimera_unitsize * block_col * block_row ) * b_t + ( info.chimera_unitsize * block_col ) * b_r
      ...
      template<std::size_t block_row, std::size_t block_col, std::size_t block_trot>
      __host__ __device__ __forceinline__ std::uint64_t
      bkIdx_ext( ChimeraInfo info, std::int64_t b_r, std::int64_t b_c, std::int64_t i, std::int64_t b_t ) {
