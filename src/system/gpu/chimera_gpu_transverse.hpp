...
namespace openjij {
  namespace system {
    ...
    struct ChimeraTransverseGPU {
      ...
      ChimeraTransverseGPU(
          ...
          spin( utility::cuda::make_dev_unique<std::int32_t[]>(
              init_interaction.get_num_row() * init_interaction.get_num_column() * info.chimera_unitsize
              * init_trotter_spins.size() ) ),
          grid( dim3(
              init_interaction.get_num_column() / cols_per_block,
      ...
      void reset_spins( const TrotterSpins& init_trotter_spins ) {
        ...
        // copy to gpu
        HANDLE_ERROR_CUDA( cudaMemcpy(
            spin.get(), temp_spin.get(), localsize * info.trotters * sizeof( int32_t ), cudaMemcpyHostToDevice ) );
      }
    ...
    private:
       ...
       */
      inline void
      initialize_gpu( const graph::Chimera<FloatType>& init_interaction, const TrotterSpins& trotter_spins, int gpu_num ) {
        ...
        // cudaMemcpy
        HANDLE_ERROR_CUDA( cudaMemcpy(
            interaction.J_out_p.get(), J_out_p.get(), localsize * sizeof( FloatType ), cudaMemcpyHostToDevice ) );
        HANDLE_ERROR_CUDA( cudaMemcpy(
            interaction.J_out_n.get(), J_out_n.get(), localsize * sizeof( FloatType ), cudaMemcpyHostToDevice ) );
        HANDLE_ERROR_CUDA( cudaMemcpy(
            interaction.J_in_04.get(), J_in_04.get(), localsize * sizeof( FloatType ), cudaMemcpyHostToDevice ) );
        HANDLE_ERROR_CUDA( cudaMemcpy(
            interaction.J_in_15.get(), J_in_15.get(), localsize * sizeof( FloatType ), cudaMemcpyHostToDevice ) );
        HANDLE_ERROR_CUDA( cudaMemcpy(
            interaction.J_in_26.get(), J_in_26.get(), localsize * sizeof( FloatType ), cudaMemcpyHostToDevice ) );
        HANDLE_ERROR_CUDA( cudaMemcpy(
            interaction.J_in_37.get(), J_in_37.get(), localsize * sizeof( FloatType ), cudaMemcpyHostToDevice ) );
        HANDLE_ERROR_CUDA(
            cudaMemcpy( interaction.h.get(), h.get(), localsize * sizeof( FloatType ), cudaMemcpyHostToDevice ) );
    ...
    ChimeraTransverseGPU<FloatType, rows_per_block, cols_per_block, trotters_per_block> make_chimera_transverse_gpu(
        ...
        double gamma,
        int device_num = 0 ) {
      return ChimeraTransverseGPU<FloatType, rows_per_block, cols_per_block, trotters_per_block>(
