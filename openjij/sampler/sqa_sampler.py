...
class SQASampler(BaseSampler):
    ...
    def _post_save(self, system, model, response):
    ...
    def sampling(self, model,
        ...
        if initial_state is None:
            sqa_system = cxxjij.system.make_transverse_ising_Eigen(
                [ising_graph.gen_spin()
                 for _ in range(self.trotter)], ising_graph, self.gamma
            )
        else:
            if model.var_type == openjij.SPIN:
                ...
                trotter_init_state = [
                    ...
                    for _ in range(self.trotter)]
            sqa_system = cxxjij.system.make_transverse_ising_Eigen(
                trotter_init_state, ising_graph, self.gamma
            )
        ...
        response = self._sampling(
            model, ising_graph, algorithm, sqa_system, initial_state,
