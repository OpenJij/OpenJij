...
public:
  ...
  void Flip(const std::int32_t index) {
    ...
    for (const auto &index_key : adjacency_list_[index]) {
      const ValueType val =
          -2 * key_value_list_[index_key].second * term_prod_[index_key];
  ...
  //! @return The sample.
  const std::vector<VariableType> &ExtractSample() const { return sample_; }
...
private:
  ...
  void SetBaseEnergyDifference() {
    ...
    for (std::size_t i = 0; i < key_value_list_.size(); ++i) {
      ...
      for (const auto &index : key_value_list_[i].first) {
        base_energy_difference_[index] +=
            value * term_prod_[i] * sample_[index];
      }
    }
  }
};
