    ...
    if (std::holds_alternative<std::int32_t>(v)) {
      ...
      for (const auto &i : variant_vec) {
        if (std::holds_alternative<std::int32_t>(i)) {
          hash ^= std::hash<std::int32_t>()(std::get<std::int32_t>(i)) +
                  0x9e3779b9 + (hash << 6) + (hash >> 2);
        } else if (std::holds_alternative<std::string>(i)) {
          hash ^= std::hash<std::string>()(std::get<std::string>(i)) +
                  0x9e3779b9 + (hash << 6) + (hash >> 2);
        } else {
...
struct VectorHash {
  template <class T> std::size_t operator()(const std::vector<T> &v) const {
    std::size_t hash = v.size();
    for (const auto &i : v) {
