...
template <typename FloatType> class IsingPolynomialModel {
...
public:
  ...
  //! @brief The variable type, which here represents binary variables \f$
  //! x_i\in \{0, 1\} \f$
  ...
  IsingPolynomialModel(std::vector<std::vector<IndexType>> &key_list,
    ...
    for (std::size_t i = 0; i < key_list.size(); ++i) {
      if (std::abs(value_list[i]) > std::numeric_limits<ValueType>::epsilon()) {
        ...
        const std::int32_t key_size =
            static_cast<std::int32_t>(key_list[i].size()) - 1;
    ...
    std::unordered_map<std::vector<std::int32_t>, ValueType,
                       utility::VectorHash>
        poly;
    ...
    // Apply threshold to avoid extremely minimum interaction derived from
    // numerical errors.
    ValueType relevant_abs_min_interaction =
        abs_max_interaction * min_max_energy_difference_ratio_;
    ...
    for (std::int32_t i = 0; i < system_size_; ++i) {
      ...
      for (const auto &interaction_index : adjacency_list_[i]) {
        if (std::abs(key_value_list_[interaction_index].second) >=
            relevant_abs_min_interaction) {
          abs_row_sum_interaction +=
              std::abs(key_value_list_[interaction_index].second);
          if (2 * std::abs(key_value_list_[interaction_index].second) <
              estimated_min_energy_difference_) {
            estimated_min_energy_difference_ =
                2 * std::abs(key_value_list_[interaction_index].second);
          }
        }
      }
  ...
  //! @return The degree.
  std::int32_t GetDegree() const { return degree_; }
  ...
  const std::vector<IndexType> &GetIndexList() const { return index_list_; }

  //! @brief Get the mapping from the index to the integer.
  //! @return The index map.
  const std::unordered_map<IndexType, std::int32_t, IndexHash> &
  GetIndexMap() const {
    return index_map_;
  }
  ...
  const std::vector<std::pair<std::vector<std::int32_t>, ValueType>> &
