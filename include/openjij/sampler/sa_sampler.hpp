...
template <class ModelType> class SASampler {
...
public:
  ...
  //! @param num_threads The number of threads in the calculation, which must be
  //! larger than zero.
  ...
  //! @param beta_min The minimum inverse temperature, which must be larger than
  //! zero.
  ...
  //! @brief Set the minimum inverse temperature.
  //! @param beta_max The minimum inverse temperature, which must be larger than
  //! zero.
  ...
  void SetRandomNumberEngine(
      const algorithm::RandomNumberEngine random_number_engine) {
  ...
  std::vector<ValueType> CalculateEnergies() const {
    if (samples_.size() == 0) {
      throw std::runtime_error("The sample size is zero. It seems that "
...
#pragma omp parallel for schedule(guided) num_threads(num_threads_)
  ...
  void Sample() { Sample(std::random_device()()); }
  ...
  void Sample(const std::uint64_t seed) {
    ...
    if (random_number_engine_ == algorithm::RandomNumberEngine::XORSHIFT) {
      TemplateSampler<system::SASystem<ModelType, utility::Xorshift>,
                      utility::Xorshift>();
    } else if (random_number_engine_ == algorithm::RandomNumberEngine::MT) {
      TemplateSampler<system::SASystem<ModelType, std::mt19937>,
                      std::mt19937>();
    } else if (random_number_engine_ == algorithm::RandomNumberEngine::MT_64) {
      TemplateSampler<system::SASystem<ModelType, std::mt19937_64>,
                      std::mt19937_64>();
    } else {
...
private:
  ...
  std::vector<
      std::pair<typename RandType::result_type, typename RandType::result_type>>
  GenerateSeedPairList(const typename RandType::result_type seed,
                       const std::int32_t num_reads) const {
    RandType random_number_engine(seed);
    std::vector<std::pair<typename RandType::result_type,
                          typename RandType::result_type>>
        seed_pair_list(num_reads);
  ...
  template <class SystemType, class RandType> void TemplateSampler() {
    const auto seed_pair_list = GenerateSeedPairList<RandType>(
        static_cast<typename RandType::result_type>(seed_), num_reads_);
    std::vector<ValueType> beta_list =
        utility::GenerateBetaList(schedule_, beta_min_, beta_max_, num_sweeps_);
...
template <class ModelType> auto make_sa_sampler(const ModelType &model) {
